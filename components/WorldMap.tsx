...
export default function WorldMap3D() {
    ...
    if (!isMounted) return (
        <div className="h-[650px] w-full flex items-center justify-center bg-black/40 backdrop-blur-md rounded-3xl border border-white/5">
            <div className="text-center space-y-4">
                <div className="w-12 h-12 border-4 border-indigo-500/30 border-t-indigo-500 rounded-full animate-spin mx-auto" />
                <p className="text-zinc-500 text-sm font-medium animate-pulse">Sincronizando Nodos Globales...</p>
            </div>
        </div>
    );

    return (
        <div className="flex flex-col lg:flex-row gap-6 w-full max-w-[1400px] mx-auto animate-fade-in relative z-20">
            {/* GLOBE SECTION */}
            <div ref={containerRef} className="h-[650px] flex-1 rounded-3xl overflow-hidden glass-morphism relative group border border-white/5 order-2 lg:order-1">
